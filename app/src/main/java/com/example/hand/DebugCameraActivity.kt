    ...
    class DebugCameraActivity : AppCompatActivity() {
        ...
        private fun hasPerson(r: HolisticLandmarkerResult): Boolean {
            ...
            for (lm in pose) {
                ...
                sumVis += vis
                if (vis >= 0.15f) good++ // ลดเหลือ 0.15
            }
            val meanVis = sumVis / pose.size
            // ลดเกณฑ์ค่าเฉลี่ยและจำนวนจุดที่ดี
            return meanVis >= 0.15f && good >= 8
        }
    
        // 🔥🔥🔥 แก้ไข: ขยายขอบเขตและลดเกณฑ์แขน 🔥🔥🔥
        private fun hasBothArms(r: HolisticLandmarkerResult): Boolean {
            ...
            fun ok(idx: Int, visTh: Float): Boolean {
                val lm = pose[idx]
                val vis = try { lm.visibility().orElse(0f) } catch (_: Throwable) { 0f }
                // ขยายขอบเขต (Padding) และลดเกณฑ์ visibility
                val inFrame = lm.x() in -0.15f..1.15f && lm.y() in -0.15f..1.15f
                return inFrame && vis >= visTh
            }
            // ลดเกณฑ์จาก 0.35 เหลือ 0.15
            val vS = 0.15f; val vE = 0.15f; val vW = 0.15f
